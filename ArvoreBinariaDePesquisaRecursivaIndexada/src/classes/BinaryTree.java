package classes;

public class BinaryTree {

    private Node root;

    public BinaryTree() {
        this.root = null;
    }

    public void insert(Item externalItem) {
        this.root = this.insert(externalItem, this.root);
    }

    public Item search(Item externalItem) {
        return this.search(externalItem, this.root);
    }

    private Node insert(Item externalItem, Node n) {
        if (n == null) {
            n = new Node();
            n.item = externalItem;
            n.item.nodeDepth = Globals.currentInsertDepth;
            System.out.println("Item de valor " + n.item.key + " adicionado");
        } else if (externalItem.key < n.item.key) {
            Globals.currentInsertDepth++;
            n.left = insert(externalItem, n.left);
        } else if (externalItem.key > n.item.key) {
            Globals.currentInsertDepth++;
            n.right = insert(externalItem, n.right);
        } else {
            System.out.println("Erro: Registro ja existente no nivel " + Globals.currentInsertDepth);
        }
        Globals.currentInsertDepth = 0;
        return n;
    }

    private Item search(Item externalItem, Node n) {
        if (n == null) {
            Item notFound = new Item(0);
            notFound.nodeDepth = Globals.currentSearchDepth;
            Globals.currentSearchDepth = 0;
            return notFound;
        } else if (externalItem.key < n.item.key) {
            Globals.currentSearchDepth++;
            return search(externalItem, n.left);
        } else if (externalItem.key > n.item.key) {
            Globals.currentSearchDepth++;
            return search(externalItem, n.right);
        } else {
            return n.item;
        }
    }

}